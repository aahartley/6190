...
void pba::Translate( const Vector& delta, DynamicalState& d )
{
#pragma omp parallel for
   for( size_t i=0;i<d->nb();i++){ d->set_pos(i, d->pos(i) + delta ); }
}

void pba::Scale( const Vector& reference, const Vector& scale, DynamicalState& d )
{
#pragma omp parallel for
   for( size_t i=0;i<d->nb();i++)
   { 
      Vector P = d->pos(i) - reference;
      P[0] *= scale[0];
      P[1] *= scale[1];
      P[2] *= scale[2];
      P += reference;
...
void pba::Rotate( const Vector& reference, const Vector& rotor, DynamicalState& d )
{
   Matrix rot = rotation( rotor.unitvector(), rotor.magnitude() );
#pragma omp parallel for
   for( size_t i=0;i<d->nb();i++)
   { 
      Vector P = d->pos(i) - reference;
